...
export async function run(cmd: NonNullable<DirOrFixture['cmd']>, { cwd, onFixture, onTestPackage }: DirOrFixture) {
  if (onTestPackage) {
...
export async function findEmberTry(): Promise<string[]> {
  return (await fs.readdir(testPackagesFolder, { withFileTypes: true }))
    .filter((stat) => stat.isDirectory())
    .map((stat) => stat.name).filter(name => name.includes('ember-try'));
}
